...
`lenses/tools/lens.py` is the reason this is practical. Tuning twenty numbers
without seeing them is guesswork.

```
# Would the app accept these?
python3 lenses/tools/lens.py check lenses/lenses.json
...
```
python3 lenses/tools/lens.py verify lenses/tools/flutter-probe.json
...
`lenses/lenses.json` is the source of truth; the app reads it from storage.
Until there is a pipeline, upload it by hand:

```
supabase storage cp lenses/lenses.json \
  ss:///kyron-media/lenses/lenses.json --experimental
```

Adding a lens is: write it, `check` it, `preview` it, commit it, upload it.
No release.
...
download one. See [AR.md](AR.md) for where the line currently sits.
