 ...
 * `process.env.JWT_SECRET` ad hoc is what allowed the verifier in
 * `jwt.strategy.ts` to fall back to a hardcoded literal while the signers had
 * no fallback at all: with JWT_SECRET unset, the API would accept any token
 * minted with a string published in this repository.
 */
